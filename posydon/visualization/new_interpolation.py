...
class EvaluateIFInterpolator:
    ...
    """
    def __init__(self, interpolator, test_grid, sn_method = "S1_SN_MODEL_v2_01_SN_type"):
        ...
        self.in_keys = interpolator.in_keys
        self.out_keys = interpolator.continuous_out_keys
        self.sn_method = sn_method
    ...
    def __compute_errs(self):
        ...
        i, c, _ = self.interpolator.evaluate(iv) # interpolated
        ivalid_inds = np.where(c[:, 0] != "initial_MT")[0]
        fv = fv[ivalid_inds]

        self.errs = {}
        nans_mask = np.isnan(fv) + np.isnan(i)

        with np.errstate(divide='ignore', invalid='ignore'):
            self.errs["relative"] = np.abs((fv - i) / (fv + eps))
        ...
        _, c, _ = self.interpolator.evaluate(iv[cvalid_inds]) # classifying

        # computing confusion matrices
        self.matrices = {}

        for i, value in enumerate(c.T):
    ...
    def __find_labels(self, key):
        ...
        labels = np.unique(self.interpolator.training_grid["final_classes"][key])

        return labels

    def __clean_errs(self, errs):
        nans = np.isnan(errs).any(axis = 1)
        infs = np.isinf(errs).any(axis = 1)

        # errs += 1.0e-16
        # errs = np.array(np.log10(errs[(~np.isnan(errs)) & (~np.isinf(errs))]), dtype = object) # dropping nans and infs
        errs = errs[(~np.isnan(errs).any(axis = 1)) & (~np.isinf(errs).any(axis = 1))]
    ...
    def violin_plots(self, err_type = "relative", keys = None,
        ...
        dirty_errs = self.errs[err_type][:, k_inds]
        errs = np.log10(self.__clean_errs(self.errs[err_type][:, k_inds]) + 1.0e-16)

        n_tracks = self.test_grid.final_values["star_1_mass"].shape[0]


        stable_inds = np.where(self.ic[self.errs["valid_inds"]] == "stable_MT")
        no_inds = np.where(self.ic[self.errs["valid_inds"]] == "no_MT")
        unstable_inds = np.where(self.ic[self.errs["valid_inds"]] == "unstable_MT")
        #TODO: add interpolation class "stable_reverse_MT"

        stable_errs = np.log10(self.__clean_errs(self.errs[err_type].T[k_inds].T[stable_inds]) + 1.0e-16)
        no_errs = np.log10(self.__clean_errs(self.errs[err_type].T[k_inds].T[no_inds]) + 1.0e-16)
        unstable_errs = np.log10(self.__clean_errs(self.errs[err_type].T[k_inds].T[unstable_inds]) + 1.0e-16)
        ...
        unstable_plot = axs.violinplot(unstable_errs, showmedians = True, points = 1000)
    ...
    def classifiers(self):
        ...
        _, c, _ = self.interpolator.evaluate(
            np.array([
    ...
    def keys(self):
        ...
        return self.interpolator.continuous_out_keys
