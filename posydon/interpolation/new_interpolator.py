...
class IFInterpolator:
    ...
    def interpolate(self, iv, klass, sn_model):
        ...
        for key, c in zip(classification_schemes, klass): # interpolating based in mass transfer type and supernova outcome separately
            ...
            if self.is_training:
                i_values = self.__normalization_utils["transform"].unnormalize(i_values[np.newaxis, ...])
            else:
                i_values = self.out_scalers[key]["transform"][c].unnormalize(i_values[np.newaxis, ...])
    ...
    def get_nearest_neighbor(self, iv, key):
        ...
        """
        normalized_initial_values = self.training_grid["normalized_initial_values"]
        dists = np.sqrt(np.square(normalized_initial_values - iv).sum(axis = 1))
