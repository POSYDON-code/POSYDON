"""Default prescriptions for wind mass loss in binary evolution."""
...
def default_spin_from_winds(a, e, primary, secondary, verbose = False):
    ...
    """Calculate the change in angular rotation rates from wind loss.

    Parameters
    ----------
    a : float
        The current orbital separation (unused in calculation, and only
        present in debugging output). [Rsolar]
    e : float
        The current orbital eccentricity (unused in calculation, and only
        present in debugging output).
...
def default_sep_from_winds(a, e, primary, secondary, verbose = False):
    ...
    """Calculate the change in orbital separation from wind loss.

    As e.g., in:

        Tauris, T. M., & van den Heuvel, E. 2006, Compact stellar X-ray
        sources, 1, 623

    From simple arguments regarding the balance of orbital angular momentum.
    ...
    -------
    da_mt_tot : float
        The change in orbital separation from both stars for a time step in
