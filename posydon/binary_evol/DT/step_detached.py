...
class detached_step:
    ...
    def __call__(self, binary):
        ...
        @event(True, 1)
        def ev_rlo1(t, y, interp1d_pri, interp1d_sec):
            ...
            """
            pri_mass = interp1d_pri["mass"](t - self.t_offset_pri)
            sec_mass = interp1d_sec["mass"](t - self.t_offset_sec)
            ...
            return interp1d_sec["R"](t - self.t_offset_sec) - 0.95*RL

        @event(True, 1)
        def ev_rlo2(t, y, interp1d_pri, interp1d_sec):
            """Difference between radius and Roche lobe at a given time.
            ...
            return interp1d_pri["R"](t - self.t_offset_pri) - 0.95*RL

        @event(True, 1)
        def ev_rel_rlo1(t, y, interp1d_pri, interp1d_sec):
            ...
            return (interp1d_sec["R"](t - self.t_offset_sec) - RL) / RL

        @event(True, 1)
        def ev_rel_rlo2(t, y, interp1d_pri, interp1d_sec):
            """Relative difference between radius and Roche lobe.
            ...
            return (interp1d_pri["R"](t - self.t_offset_pri) - RL) / RL

        @event(True, -1)
        def ev_max_time1(t, y, interp1d_pri, interp1d_sec):
            return t_max_sec + self.t_offset_sec - t

        @event(True, -1)
        def ev_max_time2(t, y, interp1d_pri, interp1d_sec):
            return t_max_pri + self.t_offset_pri - t
        ...
        self.t_offset_sec = binary.time - t0_sec
        self.t_offset_pri = binary.time - t0_pri
        max_time = interp1d_sec["max_time"]


        if (ev_rlo1(binary.time, [binary.separation, binary.eccentricity], interp1d_pri, interp1d_sec) >= 0
                or ev_rlo2(binary.time, [binary.separation, binary.eccentricity], interp1d_pri, interp1d_sec) >= 0):
            ...
            with np.errstate(all="ignore"):
                ...
                try:
                    s = solve_ivp(self.diffeq,
                        ...
                        args=(interp1d_pri, interp1d_sec),
                        dense_output=True,
                        # vectorized=True
                    )
                except Exception:
            ...
            interp1d_sec["porb"] = orbital_period_from_separation(
                sep_interp, mass_interp_sec(t - self.t_offset_sec),
                mass_interp_pri(t - self.t_offset_pri))
            interp1d_pri["porb"] = orbital_period_from_separation(
                sep_interp, mass_interp_pri(t - self.t_offset_pri),
                mass_interp_sec(t - self.t_offset_sec))
                ...
                for key in prop:
                    if key in ["event",
                        ...
                        omega_crit_current_sec = np.sqrt(const.standard_cgrav
                            * interp1d_sec[self.translate["mass"]](t[-1] - self.t_offset_sec).item() * const.msol
                            / (interp1d_sec[self.translate["R"]](t[-1] - self.t_offset_sec).item() * const.rsol)**3)
                        
                        omega_crit_hist_sec = np.sqrt(const.standard_cgrav
                            * interp1d_sec[self.translate["mass"]](t[:-1] - self.t_offset_sec) * const.msol
                            / (interp1d_sec[self.translate["R"]](t[:-1] - self.t_offset_sec) * const.rsol)**3)
                        ...
                        if primary.co:
                            ...
                            omega_crit_current_pri = np.sqrt(const.standard_cgrav
                                * interp1d_pri[self.translate["mass"]](t[-1] - self.t_offset_pri).item() * const.msol
                                / (interp1d_pri[self.translate["R"]](t[-1] - self.t_offset_pri).item() * const.rsol)**3)

                            omega_crit_hist_pri = np.sqrt(const.standard_cgrav
                                * interp1d_pri[self.translate["mass"]](t[:-1] - self.t_offset_pri) * const.msol
                                / (interp1d_pri[self.translate["R"]](t[:-1] - self.t_offset_pri) * const.rsol)**3)
                        ...
                        current = interp1d_sec[self.translate[key]](
                            t[-1] - self.t_offset_sec).item() * (const.msol * const.rsol**2)
                        history = interp1d_sec[self.translate[key]](
                            t[:-1] - self.t_offset_sec) * (const.msol * const.rsol**2)
                        ...
                        if primary.co:
                            ...
                            current = interp1d_pri[self.translate[key]](
                                t[-1] - self.t_offset_pri).item() * (const.msol * const.rsol**2)
                            history = interp1d_pri[self.translate[key]](
                                t[:-1] - self.t_offset_pri) * (const.msol * const.rsol**2)
                        ...
                        current = np.log10(
                            (current_omega / const.secyer)
                                * (interp1d_sec[
                                    self.translate["total_moment_of_inertia"]](t[-1] - self.t_offset_sec).item() * 
                        ...
                        current = (const.clight
                            ...
                            * interp1d_sec[self.translate["total_moment_of_inertia"]](
                            ...
                            / (const.standard_cgrav * (interp1d_sec[self.translate["mass"]](
                                    t[-1] - self.t_offset_sec).item() * const.msol)**2))
                        history = (const.clight
                            * (interp1d_sec["omega"][:-1] / const.secyer)
                            * interp1d_sec[self.translate["total_moment_of_inertia"]](
                                    t[:-1] - self.t_offset_sec)* (const.msol * const.rsol**2)
                            / (const.standard_cgrav * (interp1d_sec[self.translate["mass"]](
                                    t[:-1] - self.t_offset_sec) * const.msol)**2))
                            ...
                            current = (const.clight
                                ...
                                * interp1d_pri[self.translate["total_moment_of_inertia"]](
                                ...
                                / (const.standard_cgrav * (interp1d_pri[self.translate["mass"]](
                                        t[-1] - self.t_offset_pri).item() * const.msol)**2))
                            history = (const.clight 
                                * (interp1d_pri["omega"][:-1] / const.secyer)
                                * interp1d_pri[self.translate["total_moment_of_inertia"]](
                                        t[:-1] - self.t_offset_pri) * (const.msol * const.rsol**2)
                                / (const.standard_cgrav * (interp1d_pri[self.translate["mass"]](
                                        t[:-1] - self.t_offset_pri) * const.msol)**2))
                            
                    elif (key in ["lg_mdot", "lg_wind_mdot"] and obj == secondary):
                        # in detached step, lg_mdot = lg_wind_mdot
                        if interp1d_sec[self.translate[key]](t[-1] - self.t_offset_sec) == 0:
                            current = -98.99
                        else:
                            current = np.log10(np.abs(interp1d_sec[self.translate[key]](
                                t[-1] - self.t_offset_sec))).item()
                            
                        history = np.ones_like(t[:-1])

                        for i in range(len(t)-1):
                            if interp1d_sec[self.translate[key]](t[i] - self.t_offset_sec) == 0:
                                history[i] = -98.99
                            else:
                                history[i] = np.log10(np.abs(interp1d_sec[self.translate[key]](
                                        t[i] - self.t_offset_sec)))
                        ...
                        current = interp1d_sec[self.translate[key]](t[-1] - self.t_offset_sec).item()
                        history = interp1d_sec[self.translate[key]](t[:-1] - self.t_offset_sec)
                        ...
                        if primary.co:
                            ...
                            current = interp1d_pri[self.translate[key]](t[-1] - self.t_offset_pri).item()
                            history = interp1d_pri[self.translate[key]](t[:-1] - self.t_offset_pri)
            ...
            ## CHECK IF STARS WILL UNDERGO CC
                ...
                if not primary.co and primary.state in STAR_STATES_CC:
                    ...
                    primary_time = t_max_pri + self.t_offset_pri - t[-1]
                    secondary_time = t_max_sec + self.t_offset_sec - t[-1]
    ...
    def diffeq(self, t, y, interp1d_pri, interp1d_sec):
        """
        ...
        R_sec = interp1d_sec["R"](t - self.t_offset_sec) # Rsol
        L_sec = interp1d_sec["L"](t - self.t_offset_sec) # Lsol
        M_sec = interp1d_sec["mass"](t - self.t_offset_sec) # Msol
        Mdot_sec = interp1d_sec["mdot"](t - self.t_offset_sec) # Msol/yr
        I_sec = interp1d_sec["inertia"](t - self.t_offset_sec) # Msol*Rsol^2
        conv_mx1_top_r_sec = interp1d_sec["conv_mx1_top_r"](t - self.t_offset_sec) # Rsol
        conv_mx1_bot_r_sec = interp1d_sec["conv_mx1_bot_r"](t - self.t_offset_sec) # Rsol
        surface_h1_sec = interp1d_sec["surface_h1"](t - self.t_offset_sec) # dex
        center_h1_sec = interp1d_sec["center_h1"](t - self.t_offset_sec) # dex
        M_env_sec = interp1d_sec["mass_conv_reg_fortides"](t - self.t_offset_sec) # Msol
        DR_env_sec = interp1d_sec["thickness_conv_reg_fortides"](t - self.t_offset_sec) # Rsol
        Renv_middle_sec = interp1d_sec["radius_conv_reg_fortides"](t - self.t_offset_sec) # Rsol
        Idot_sec = interp1d_sec["Idot"](t - self.t_offset_sec) # Msol*Rsol^2/yr
        tau_conv_sec = interp1d_sec["conv_env_turnover_time_l_b"](t - self.t_offset_sec) # yr

        # primary star properties used in evolution
        R_pri = interp1d_pri["R"](t - self.t_offset_pri)
        L_pri = interp1d_pri["L"](t - self.t_offset_pri)
        M_pri = interp1d_pri["mass"](t - self.t_offset_pri)
        Mdot_pri = interp1d_pri["mdot"](t - self.t_offset_pri)
        I_pri = interp1d_pri["inertia"](t - self.t_offset_pri)
        conv_mx1_top_r_pri = interp1d_pri["conv_mx1_top_r"](t - self.t_offset_pri)
        conv_mx1_bot_r_pri = interp1d_pri["conv_mx1_bot_r"](t - self.t_offset_pri)
        surface_h1_pri = interp1d_pri["surface_h1"](t - self.t_offset_pri)
        center_h1_pri = interp1d_pri["center_h1"](t - self.t_offset_pri)
        M_env_pri = interp1d_pri["mass_conv_reg_fortides"](t - self.t_offset_pri)
        DR_env_pri = interp1d_pri["thickness_conv_reg_fortides"](t - self.t_offset_pri)
        Renv_middle_pri = interp1d_pri["radius_conv_reg_fortides"](t - self.t_offset_pri)
        Idot_pri = interp1d_pri["Idot"](t - self.t_offset_pri)
        tau_conv_pri = interp1d_pri["conv_env_turnover_time_l_b"](t - self.t_offset_pri)
