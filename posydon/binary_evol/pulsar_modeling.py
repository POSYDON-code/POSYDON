...
class Pulsar:
    ...
    def detached_evolve(self, delta_t):
        ...
        omega_f = np.sqrt(1/(8*np.pi*R**6*np.sin(alpha)**2/(3*mu_0*const.clight**3*I) * B_min**2*delta_t + 1/self.spin**2)) 
        self.spin = omega_f
    ...
    def CE_evolve(self):
        ...
        ## params needed for RLO evolve
        delta_t = 0     ## assume CE phase is instantaneous
        T = 0           

        self.RLO_evolve(delta_t, T, delta_M)


    def check_death_line(self):
        '''
