...
POSYDON shares the precomputed MESA simulation grids, together with their
...
Because the code evolved substantially between v1 and v2 (the grid types,
metallicity coverage, and post-processing all changed), DR1 is not the default
...
contains a fixed grid of single- and binary-star MESA simulations processed
...
===================== =================
Full DR2 dataset       ~ 140 GB
Each DR2 metallicity   ~ 20 GB
DR1 dataset            ~ 40 GB
===================== =================

.. note::

   Earlier versions of this documentation quoted slightly different numbers
   (such as ~ 103 GB) depending on whether the compressed or single-metallicity
   subset was being totaled. The authoritative numbers are the ones in the
   table above: plan for about 140 GB for a full DR2 download and about 20 GB
   per metallicity.

The exact footprint also depends on how many run types (``HMS-HMS``, ``He-He``,
and so on; see :ref:`grid-types`) and which auxiliary files (the interpolation
objects) you include.
